...
class PrintlnStrategyTests {
    ...
    fun printlnStrategy_should_print_string_literal() {
        // Arrange
        val literalNode = LiteralNode("\"hello world\"", object : org.example.TokenType { override val name = "STRING" })
        val printNode = PrintlnNode(literalNode)
        
        var capturedOutput = ""
        val mockOutput = object : Output {
        ...
        assertNull(result) // println no devuelve valor
    }
    ...
    fun printlnStrategy_should_print_decimal_number() {
        // Arrange
        val literalNode = LiteralNode("3.14", object : org.example.TokenType { override val name = "NUMBER" })
        val printNode = PrintlnNode(literalNode)
    ...
    fun printlnStrategy_should_print_empty_string() {
        // Arrange
        val literalNode = LiteralNode("\"\"", object : org.example.TokenType { override val name = "STRING" })
        val printNode = PrintlnNode(literalNode)
