...
class DefaultLexer(private val tokenProvider : TokenProvider) : Lexer {
        ...
        override fun tokenize(sourceCode: String): List<Token> {
            return tokenizeRecursively(sourceCode, 0, 1, 1 , emptyList())

        }

        private fun tokenizeRecursively(sourceCode: String, index: Int, line: Int, column: Int, tokens: List<Token>): List<Token> {
            if (index >= sourceCode.length) {
                return tokens
            }
            val match = firstToMatch(sourceCode, index)
    ...
    private fun updateLineAndColumn(tokenValue: String, line: Int, column: Int): Pair<Int, Int> {
    ...
    private fun firstToMatch(sourceCode: String, index: Int): Pair<TokenRule, String>? {
